...
class RequestHandler extends BaseController
{
    ...
    public function validateRequest($class, $method, $requestMethod, $payload = [], $classObject = null, $splitPath = [])
    {
        ...
        if ($shouldAuthenticate || $partialAuthenticate) {
            ...
            // confirm that the user role is in the list of endpoint roles
            if(isset($routes[$method]['roles']) && !in_array($userToken['role'], $routes[$method]['roles'])) {
                return Routing::denied('You do not have permission to access this endpoint.');
            }
            ...
            $this->setCurrentUser($userToken);

            // if the user does not have access to the module, return an error
            if(!$this->canAccessModule($class)) {
                return Routing::denied('You do not have permission to access this module.');
            }

            // if the user does not have permission to access the method, return an error
            if(!$this->hasPermission($class, $method)) {
            ...
            foreach(['assembly_id', 'contractor_id', 'jurisdiction_id', 'household_id'] as $key) {
                if(isset($routes[$method]['payload'][$key]) && empty($payload[$key])) {
                    $payload[$key] = $userToken[$key];
                }
            }
        }
