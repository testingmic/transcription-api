...
class Api extends BaseController
{
    ...
    private function processHandler($class, $method = 'list', $payload = [], $uniqueId = '') {
        ...
        if(!class_exists($classname)) {
            ...
            return [
                'status' => 'error',
                'message' => "Sorry! The model '{$class}' you are trying to access is unavailable."
            ];
        }
        ...
        // convert the and  to integers
        foreach(['branchId', 'userId', 'pharmacyId', 'offset'] as $key) {
            if(isset($payload[$key])) {
        ...
        try {
            ...
            if(isset($validObject->cacheObject) && !empty($validObject->cacheObject->currentUser)) {
                $classObject->theCurrentUser = $validObject->cacheObject->currentUser;
            }
            ...
            $this->statusCode = 500;
            return Routing::error("Error: ". $e->getMessage() . " in file: " . $e->getFile() . " on line: " . $e->getLine());
        }
